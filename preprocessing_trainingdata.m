...
for i = 3:length(imlist)
    ...
    if sum(str2double(filenum)) == 29 || sum(str2double(filenum)) == 49 || sum(str2double(filenum)) == 51 || sum(str2double(filenum)) == 52 || sum(str2double(filenum)) == 62 || sum(str2double(filenum)) == 66 || sum(str2double(filenum)) == 76 || sum(str2double(filenum)) == 82 || sum(str2double(filenum)) == 83 || sum(str2double(filenum)) == 87
        state = 'val';
    else
        state = 'train';
    end
    ...
    % read image
    img = imread([folder '/HeadEyeImages/' im_filename]);
    imw = size(img, 2);
    iml = size(img, 1);
    
    % read salmap
    fileId = fopen([folder '/HeadEyeSalMaps_bin/HEsalmap_' filenum '_2048x1024_32b.bin'], 'rb');
    ...
    M = csvread([folder '/HeadEyeScanpath_txt/HEscanpath_' filenum '.txt'], 1, 0);
    flen = size(M,1);
    for k = 1:flen-1
        fixpos = round(M(k, 3:4))+[1 1];
%		fixpos = single(fixpos);
